...
YOLOv8m-pose + RTMPose를 사용해 클라이밍 영상에서 골격(skeleton) 오버레이 영상을 생성합니다.  
...
| `yolo_pose_analyze.py` | YOLOv8m-pose 포즈 분석 로컬 스크립트 |
| `mediapipe_analyze.py` | RTMPose 분석 로직 (modal 워커에서 임포트) |

## 동작 방식

1. 클라이언트(API 서버)가 `.mp4` 영상을 multipart POST로 전송
2. Modal GPU 워커가 YOLOv8m-pose + RTMPose Hand로 골격 감지
3. 프레임별 17 body + 42 hand 키포인트 오버레이 영상 생성
...
- **RTMPose** (rtmlib): [Apache 2.0](https://github.com/Tau-J/rtmlib/blob/main/LICENSE)
- 이 저장소: AGPL-3.0
